module CanopyStateType
  ...
  type, public :: CanopyState_type
     ...
     real*8, pointer :: tlai_patch               (:)   => null()! patch canopy one-sided leaf area index, no burying by snow
     real*8, pointer :: tsai_patch               (:)   => null()! patch canopy one-sided stem area index, no burying by snow
     real*8, pointer :: elai_patch               (:)   => null()! patch canopy one-sided leaf area index with burying by snow
     real*8, pointer :: esai_patch               (:)   => null()! patch canopy one-sided stem area index with burying by snow
     real*8, pointer :: elai_p_patch             (:)   => null()! patch canopy one-sided leaf area index with burying by snow average over timestep
     real*8, pointer :: laisun_patch             (:)   => null()! patch patch sunlit projected leaf area index
     real*8, pointer :: laisha_patch             (:)   => null()! patch patch shaded projected leaf area index
     real*8, pointer :: laisun_z_patch           (:,:) => null()! patch patch sunlit leaf area for canopy layer
     real*8, pointer :: laisha_z_patch           (:,:) => null()! patch patch shaded leaf area for canopy layer
     real*8, pointer :: mlaidiff_patch           (:)   => null()! patch difference between lai month one and month two (for dry deposition of chemical tracers)
     real*8, pointer :: annlai_patch             (:,:) => null()! patch 12 months of monthly lai from input data set (for dry deposition of chemical tracers)
     real*8, pointer :: htop_patch               (:)   => null()! patch canopy top (m)
     real*8, pointer :: hbot_patch               (:)   => null()! patch canopy bottom (m)
     real*8, pointer :: displa_patch             (:)   => null()! patch displacement height (m)
     real*8, pointer :: fsun_patch               (:)   => null()! patch sunlit fraction of canopy
     real*8, pointer :: fsun24_patch             (:)   => null()! patch 24hr average of sunlit fraction of canopy
     real*8, pointer :: fsun240_patch            (:)   => null()! patch 240hr average of sunlit fraction of canopy

     real*8, pointer :: alt_col                  (:)   => null()! col current depth of thaw
     integer , pointer :: alt_indx_col             (:)   => null()! col current depth of thaw
     real*8, pointer :: altmax_col               (:)   => null()! col maximum annual depth of thaw
     real*8, pointer :: altmax_lastyear_col      (:)   => null()! col prior year maximum annual depth of thaw
     ...
     real*8, pointer :: dewmx_patch              (:)   => null()! patch maximum allowed dew [mm]

     real*8, pointer :: dleaf_patch              (:)   => null()! patch characteristic leaf width (diameter) [m]
                                                                  ! for non-ED/FATES this is the same as pftcon%dleaf()
     real*8, pointer :: lbl_rsc_h2o_patch        (:)   => null()! laminar boundary layer resistance for water over dry leaf (s/m)
   contains
